...
class QuantileClip(Clipper):
    def __init__(
        ...
        self.global_clip_quantile = None
        self.global_history_length = None
        if self.global_threshold:
            self.global_clip_quantile = clip_quantile
            self.global_history_length = history_length

        super().__init__(
            parameters,
            {"clip_quantile": clip_quantile, "history_length": history_length},
        )
    ...
    def _clip_local(self):
        for parameter_group in self.parameter_groups:
            group_clip_quantile = parameter_group["clip_quantile"]
            ...
            for parameter in parameter_group["params"]:
                ...
                if len(state) == 0:
                    ...
                    threshold = torch.quantile(state["history"], group_clip_quantile)
    ...
    def _clip_global(self):
        ...
        if len(self.state["global_history"]) == 0:
            ...
            threshold = torch.quantile(
                self.state["global_history"], self.global_clip_quantile
            )
    ...
    def add_param_group(
        ...
        if clip_quantile is not None:
            parameter_group_args["clip_quantile"] = clip_quantile
